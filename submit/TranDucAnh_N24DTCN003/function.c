...
int findCategoryByID(char *id) {                            // pointer id de tim kiem voi id trong file
    ...
    return -1;         // khong tim thay ( cho la: -1, la gia tri khong xac dinh )
}
//----------------------------------------------------tim kiem danh muc theo ten (tuong doi)-----------------------------------------------------------------
...
void searchCategory() {
    ...
    for (int i = 0; i < category_count; i++) {        // duyet qua tat ca danh muc.
        if (strstr(categories[i].CategoryName, name) != NULL) {       // do strstr la tim chuoi con trong chuoi lon -->tim kiem tuong doi.
...
//-------------------------------------------------------tim kiem thong tin (tuyet doi)-----------------------------------------------------------------
void searchProduct() {   // binary sreach.               
    char name[MAX_NAME];
	...
	for (int i = 0; i < product_count - 1; i++) {
	        for (int j = 0; j < product_count - i - 1; j++) {
	            if (strcmp(products[j].productName, products[j + 1].productName) > 0) {  // do strcmp la so sanh toan bo chuoi -->tim kiem tuyet doi
    ...
    while (left <= right) {
	        ...
	        int cmp = strcmp(products[mid].productName, name);
	
	        if (cmp == 0) {  // tim kiem san pham.
	            printf("| %-10s | %-10s | %-30s | %-10d |\n", 
	                   products[mid].productID, products[mid].categoryID, products[mid].productName, products[mid].price);
	            printf("+------------+------------+--------------------------------+------------+\n");
	            found = 1;
	            break;
	        } else if (cmp < 0) {
...
//-----------------------------------------------------sap xep thong tin(theo gia)-----------------------------------------------------------------
void sortProducts() {         // selection sort
    int sortChoice;
    ...
    // Selection Sort
	    for (int i = 0; i < product_count - 1; i++) {   // duyet qua tung product.
	        int selectedIndex = i;
	        
	        for (int j = i + 1; j < product_count; j++) { // tu i+1 den cuoi day.
	            // Tim phan tu nho nhat (neu sortChoice = 1) hoac lon nhat (neu sortChoice = 2)
	            if ((sortChoice == 1 && products[j].price < products[selectedIndex].price) || 
	                (sortChoice == 2 && products[j].price > products[selectedIndex].price)) {
	                selectedIndex = j;  
	            }
	        }
	
	        // hoan doi phan tu tai vi tri i voi phan tu (nho/lon) tim duoc (selectedIndex).
	        if (selectedIndex != i) {
	            struct Product temp = products[i];
	            products[i] = products[selectedIndex];
	            products[selectedIndex] = temp;
	        }
	    }
...
int findProductByID(char *id) {
    if (id == NULL || product_count == 0) {
        return -1;  // Tranh loi khi tham so id là NULL hoac khong co san pham nao
    }

    for (int i = 0; i < product_count; i++) {
        if (strcmp(products[i].productID, id) == 0) {  // So sanh ID
            return i;  // Tra ve chi so san pham tim thay
        }
    }
    return -1;  // Khong tim thay san pham
}
//----------------------------------------------------loc san pham theo gia/ten danh muc--------------------------------------------------------------
void filterProducts() {
    int filterChoice;
    printf("                           Choose the filter option:\n");
    printf("                        ==================================     \n");
    printf("                           [1]. Filter by Price\n");
    printf("                           [2]. Sort categories by name\n");
    printf("                        ==================================     \n");
    printf("                           Your choice: ");
    ...
    if (filterChoice == 1) {
    	int price;
		printf("                           Enter the price to filter by: ");
		scanf("%d", &price);
		while (getchar() != '\n'); // Xoa bo nho dem
		...
		// loc san pham theo gia nguo dung nhap:
		for (int i = 0; i < product_count; i++) {
		...
		char categoryName[MAX_NAME];
		printf("                           Enter the category name to filter by: ");
		scanf("%29s", categoryName);  // Gioi han chieu dai nhap ten danh muc.
		while (getchar() != '\n'); // Xoa bo nho dem.
		printf("\n                           FILTERED PRODUCTS BY CATEGORY NAME\n");
		printf("+------------+------------+--------------------------------+------------+\n");
		printf("| %-10s | %-10s | %-30s | %-10s |\n", "ProductID", "CategoryID", "Product Name", "Price");
		printf("+------------+------------+--------------------------------+------------+\n");
		        
		// Loc va hien thi cac san pham thuoc danh muc co ten nguoi dung nhap.
		int found = 0;  // Bien kiem tra co san pham nao duoc loc khong
		for (int i = 0; i < product_count; i++) {
		// Tim danh muc cua san pham (dua vào CategoryID cua san pham)
		int categoryIndex = findCategoryByID(products[i].categoryID);
		            
		// Kiem tra neu danh muc co ten trung voi ten nguoi dung nhap
		if (categoryIndex != -1 && strstr(categories[categoryIndex].CategoryName, categoryName) != NULL) {
		    // Neu co, in ra san pham do.
		    printf("| %-10s | %-10s | %-30s | %-10d |\n", products[i].productID, products[i].categoryID, products[i].productName, products[i].price);
		    found = 1;
		}
	}
