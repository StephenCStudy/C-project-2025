...
//--------------------------------------------------------------them danh muc-------------------------------------------------------------------
	...
	void addCategoryList() {
	    ...
	    if(checkInputData(newCategory.CategoryID, newCategory.CategoryName,category_count) == 1){
...
void sortCategories() {
    ...
    for (int i = 0; i < category_count - 1; i++) {                             // bubble sort
        for (int j = 0; j < category_count -i -1; j++) {
            if ((sortChoice == 1 && strcmp(categories[j].CategoryName, categories[j+1].CategoryName) > 0) || 
			// neu j lon hon j+1 [cmp>0]--> hoan doi vitri
                (sortChoice == 2 && strcmp(categories[j].CategoryName, categories[j+1].CategoryName) < 0)) {  
...
int isUniqueName(char* name,int indexCategory) {
    for (int i = 0; i < category_count; i++) {
        if (i != indexCategory && strcmp(categories[i].CategoryName, name) == 0) {
